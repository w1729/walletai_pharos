...
export default function SendTxModal() {
  ...
  const submitTx = async (e: any) => {
    ...
    try {
      ...
      console.log("user input:", userInputAmount, price["ethereum"].usd);
      const { maxFeePerGas, maxPriorityFeePerGas }: EstimateFeesPerGasReturnType =
        await ethClient.estimateFeesPerGas();
      console.log(
        "transfer amount:",
        BigInt(parseEther(userInputAmount)) /
          (BigInt(Math.trunc(price.ethereum.usd * 100)) / BigInt(100)),
      );
      const userOp = await builder.buildUserOp({
        calls: [
          {
        ...
        <Flex direction="column" justify="center" align="center" grow="1" gap="5">
          {true ? (
            <>
              ...
              <Link
                href={`https://sepolia.swellchainscan.io/tx/${txReceipt?.receipt?.transactionHash}`}
                target="_blank"
                ...
                <Flex direction="column" gap="1">
                  <Flex direction="column" gap="2">
                    ...
                    <Text
                      ...
                      color="gray"
                    >
                      ${balance.toString().slice(0, 6)} available
                    </Text>
                  </Flex>
                </Flex>
              </div>
